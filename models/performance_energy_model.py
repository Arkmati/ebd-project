...
class ClusterModels:
    ...
    def initialize_models(self):
        ...
        try:
            iot = pd.read_csv('generated_data/historical-iot.csv', parse_dates=['Timestamp'])
            scada = pd.read_csv('generated_data/historical-scada.csv', parse_dates=['Timestamp'])
            mes = pd.read_csv('generated_data/historical-mes.csv', parse_dates=['Timestamp'])
...
def kafka_consumer_loop():
    ...
    for message in consumer:
        ...
        # Process MES data
        if message.topic == 'mes-data':
            process_mes_data(record, mid)

        # Process energy data
        elif message.topic in ['scada-data', 'iot-data']:
        ...
        if message.topic == 'mes-data' and 'Operator_ID' in record:
            operator_id = record['Operator_ID']
            key = (operator_id, mid)
        ...
        if 'Time_per_Unit' not in machine_data or 'Defect_Rate' not in machine_data:
            ...
            if 'history' in machine_data:
                ...
                for record in reversed(machine_data['history']):
                    if record.get('topic') == 'mes-data' and 'data' in record:
