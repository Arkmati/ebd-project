...
DEF_UNITS_MODEL2_PATH    = os.path.join(MODEL_DIR, 'def_units_reg2.pkl')
DEF_UNITS_PIPELINE_PATH    = os.path.join(MODEL_DIR, 'def_units_reg-pipeline.pkl')
...
# ----------------------------------------
print("Loading historical SCADA, IOT and MES data for defective units regressor...")
...
print("Merging dataframes on Timestamp & Machine_ID for defective units regressor...")
data = (
...
def load_or_fit_encoder(path, series):
    if os.path.exists(path):
        ...
        print(f"Loaded encoder {path} for defective units regressor")
    else:
        enc = LabelEncoder()
        encoded = enc.fit_transform(series)
        joblib.dump(enc, path)
        print(f"Saved new encoder {path} for defective units regressor")
    return enc, encoded

print("Encoding categorical features for defective units regressor...")
...
y = data['Defective_Units'].values

print("Bootstrapping regressors for Defective units...")
if os.path.exists(DEF_UNITS_MODEL_PATH):
    reg = joblib.load(DEF_UNITS_MODEL_PATH)
    print("Loaded persisted defective units regressor.")
else:
    ...
    print("Initialized and bootstrapped new defective units regressor.")

if os.path.exists(DEF_UNITS_MODEL2_PATH) and os.path.exists(SCALAR2_PATH):
    scaler = joblib.load(SCALAR2_PATH)
    reg2 = joblib.load(DEF_UNITS_MODEL2_PATH)
    print("Loaded persisted defective units scaled regressor with corresponding scaler")
else:
    print("Initializing defective units scaled regressor reg2...")
    ...
    print("Initialized and bootstrapped new defective units scaled regressor.")

if os.path.exists(DEF_UNITS_PIPELINE_PATH):
    reg3 = joblib.load(DEF_UNITS_PIPELINE_PATH)
    print("Loaded persisted defective units pipeline regressor.")
else:
    reg3 = make_pipeline(StandardScaler(),
        SGDRegressor(max_iter=1, tol=None, warm_start=True,
            learning_rate='invscaling', eta0=1e-3, alpha=1e-4, average=True) # added average=True
    )
    reg3.fit(X, y)
    joblib.dump(reg3, DEF_UNITS_PIPELINE_PATH)
    print("Initialized and bootstrapped new defective units pipeline regressor.")

def evaluate_model_performance(num_passes):
    print(f"Evaluating model performance on training data for defective units regressor where number of passes={num_passes}...")
    pred1 = reg.predict(X)
    ...
    for name, pred in [('reg', pred1), ('reg2', pred2), ('reg3', pred3)]:
        ...
        r2  = r2_score(y, pred)
        print(f"Defective units regressor {name} -> MSE: {mse:.2f}, MAE: {mae:.2f}, R2: {r2:.3f} when number of passes={num_passes}")

print("Evaluating model performance on training data for defective units regressor after just 1 partial_fit had been run...")
