...
def load_state(path):
    if os.path.exists(path):
        with open(path, 'rb') as f:
            st = pickle.load(f)
        print(f"Loaded persisted state ({len(st)} machines) for defective units regressor")
        return st
    print("Init empty state buffer for defective units regressor")
    return {}
...
def kafka_consumer_loop():
    ...
    for msg in consumer:
        ...
        if all(k in raw for k in ['Temperature_C','Vibration_mm_s','Pressure_bar',
            ...
            y_true = state[mid]['Defective_Units']
            if topic == 'mes-stream' and 'Defective_Units' in raw:
                y_true = raw['Defective_Units']
